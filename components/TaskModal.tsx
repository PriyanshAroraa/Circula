...
import { X, Clock, AlignLeft, Tag, Zap, AlertTriangle, AlertCircle } from 'lucide-react';
...
const TaskModal: React.FC<TaskModalProps> = ({ isOpen, onClose, onSave, initialData, onDelete, existingTasks = [] }) => {
  ...
  // Validate task whenever form data changes
  useEffect(() => {
    const taskData = {
      title,
      category,
    ...
    const result = validateTask(taskData, existingTasks, initialData?.id);
    setValidation(result);
  }, [title, category, startTime, endTime, notes, energyLevel, existingTasks, initialData?.id]);
          ...
          {(validation.errors.length > 0 || validation.warnings.length > 0) && (
            <div className="space-y-2">
              {validation.errors.map((error, index) => (
                <div key={`error-${index}`} className="flex items-center gap-2 p-3 bg-error/10 border border-error/20 rounded-lg">
                  <AlertCircle size={16} className="text-error flex-shrink-0" />
                  <span className="text-error text-sm">{error.message}</span>
                </div>
              ))}
              {validation.warnings.map((warning, index) => (
                <div key={`warning-${index}`} className="flex items-center gap-2 p-3 bg-warning/10 border border-warning/20 rounded-lg">
                  <AlertTriangle size={16} className="text-warning flex-shrink-0" />
                  <span className="text-warning text-sm">{warning.message}</span>
                </div>
              ))}
            </div>
          )}
